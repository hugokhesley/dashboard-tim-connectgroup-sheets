...
def main():
    ...
    # ── Input por dia ─────────────────────────────────────────────────────────
    st.markdown('<p class="section-title">📥 Input por Dia — Acessos</p>', unsafe_allow_html=True)

    tab_input, tab_atv, tab_backlog = st.tabs([
        "📥 Input Diário", "✅ Ativação Diária", "⏳ Backlog por Vendedor"
    ])

    with tab_input:
        if df_mes_input.empty:
            st.info("Nenhum input encontrado para o período.")
        else:
            # Agrupa por dia
            diario = (df_mes_input.groupby(df_mes_input["dt_input"].dt.day)
                      .agg(acessos=("acessos","sum"), receita=("preco_oferta","sum"),
                           pedidos=("pedido","nunique"))
                      .reset_index().rename(columns={"dt_input":"dia"}))
            ...
            for _, row in diario.iterrows():
                ...
                badge   = '<span class="hoje-badge">HOJE</span>' if eh_hoje else ""
                cor     = "#6366f1" if not eh_hoje else "#a78bfa"
                html_dias += f"""
                <div class="day-bar-wrap">
                  <div class="day-label">
                    <span>Dia {dia:02d}{badge} · {int(row['pedidos'])} pedido(s)</span>
                    <span style="font-weight:700;color:#e2e8f0">{int(row['acessos'])} ac. <span style="color:#64748b">· R$ {row['receita']:,.2f} · acum. {int(row['acumulado'])}</span></span>
                  </div>
                  {_bar(row['acessos'], max_ac, cor)}
                </div>"""
            st.markdown(f'<div style="max-height:500px;overflow-y:auto;padding-right:8px">{html_dias}</div>',
                        unsafe_allow_html=True)

            # Tabela detalhada por vendedor no dia selecionado
            st.markdown("")
            with st.expander("📋 Ver detalhe por vendedor/dia", expanded=False):
                det = (df_mes_input.groupby([df_mes_input["dt_input"].dt.day, "vendedor_real"])
                       .agg(acessos=("acessos","sum"), receita=("preco_oferta","sum"),
                            pedidos=("pedido","nunique"))
                       .reset_index().rename(columns={"dt_input":"dia"}))
    ...
    with tab_atv:
        if df_mes_atv.empty:
            st.info("Nenhuma ativação encontrada para o período.")
        else:
            diario_atv = (df_mes_atv.groupby(df_mes_atv["dt_ativacao"].dt.day)
                          .agg(acessos=("acessos","sum"), receita=("preco_oferta","sum"))
                          .reset_index().rename(columns={"dt_ativacao":"dia"}))
            diario_atv = diario_atv.sort_values("dia")
            max_atv = diario_atv["acessos"].max()
            diario_atv["acumulado"] = diario_atv["acessos"].cumsum()

            html_atv = ""
            for _, row in diario_atv.iterrows():
                dia     = int(row["dia"])
                eh_hoje = (eh_mes_atual and dia == hoje.day)
                badge   = '<span class="hoje-badge">HOJE</span>' if eh_hoje else ""
                cor     = "#10b981" if not eh_hoje else "#34d399"
                html_atv += f"""
                <div class="day-bar-wrap">
                  <div class="day-label">
                    <span>Dia {dia:02d}{badge}</span>
                    <span style="font-weight:700;color:#e2e8f0">{int(row['acessos'])} ac. <span style="color:#64748b">· R$ {row['receita']:,.2f} · acum. {int(row['acumulado'])}</span></span>
                  </div>
                  {_bar(row['acessos'], max_atv, cor)}
                </div>"""
            st.markdown(f'<div style="max-height:500px;overflow-y:auto;padding-right:8px">{html_atv}</div>',
                        unsafe_allow_html=True)

    with tab_backlog:
        # Backlog = pedidos sem data de ativação
        df_backlog = df_base[df_base["dt_ativacao"].isna()].copy()

        if df_backlog.empty:
            st.success("🎉 Sem backlog! Todos os pedidos já foram ativados.")
        else:
            total_bl = int(df_backlog["acessos"].sum())
            n_ped    = df_backlog["pedido"].nunique() if "pedido" in df_backlog.columns else len(df_backlog)
            st.markdown(f"""<div style="background:#1a1f2e;border-radius:10px;padding:14px 20px;
                border:1px solid #2d3748;margin-bottom:16px;display:flex;gap:32px">
              <div><div style="font-size:0.7rem;color:#94a3b8;text-transform:uppercase">Total Backlog</div>
                   <div style="font-size:1.6rem;font-weight:800;color:#f59e0b">{total_bl:,} ac.</div></div>
              <div><div style="font-size:0.7rem;color:#94a3b8;text-transform:uppercase">Pedidos</div>
                   <div style="font-size:1.6rem;font-weight:800;color:#f59e0b">{n_ped}</div></div>
            </div>""", unsafe_allow_html=True)

            # Por vendedor
            if "vendedor_real" in df_backlog.columns:
                por_vend = (df_backlog.groupby("vendedor_real")
                            .agg(acessos=("acessos","sum"), pedidos=("pedido","nunique"),
                                 receita=("preco_oferta","sum"))
                            .reset_index().sort_values("acessos", ascending=False))
                max_bl = por_vend["acessos"].max()

                for _, row in por_vend.iterrows():
                    with st.expander(f"👤 {row['vendedor_real']} — {int(row['acessos'])} ac. · {int(row['pedidos'])} pedido(s)", expanded=False):
                        # Pedidos deste vendedor em backlog
