...
def _render_gestao_vista(df_base, df_mes_atv, df_mes_input, mes_sel, hoje, eh_mes_atual, tema_sel, raw=None, bko=None, parceiro_sel='Todos', lider_sel='Todos'):
    ...
    col_tg1, col_tg2, col_tg3 = st.columns([2, 1, 3])
    with col_tg1:
        chat_ids_input = st.text_input(
            "Chat IDs (separados por vírgula)",
            value=st.secrets.get("telegram_gestao", {}).get("chat_ids", ""),
            placeholder="123456789,987654321",
            help="Seu chat_id e do gerente. Para descobrir: mande uma msg pro bot e acesse getUpdates.",
            key="gav_chat_ids"
        )
    with col_tg2:
    ...
    if enviar_tg:
        ...
        if not chat_ids:
            ...
            try:
                ...
                def _cpct(p):
                    if p is None: return _colors.HexColor("#94a3b8")
                    return (_colors.HexColor("#34d399") if p >= 90
                            else (_colors.HexColor("#fbbf24") if p >= 70
                                  else _colors.HexColor("#f87171")))

                CH = _colors.HexColor
                COR = {
                    "hdr_bg":  CH("#1e1b4b"), "hdr_tx":  CH("#c7d2fe"),
                    "row_a":   CH("#0f1117"), "row_b":   CH("#1a1f2e"),
                    "tot_bg":  CH("#1e1b4b"), "tot_tx":  CH("#c7d2fe"),
                    "borda":   CH("#2d3748"), "verde":   CH("#34d399"),
                    "azul":    CH("#60a5fa"), "roxo":    CH("#a78bfa"),
                    "texto":   CH("#e2e8f0"), "sub":     CH("#94a3b8"),
                    "acc":     CH("#6366f1"),
                }
                ...
                agora_pdf  = datetime.now().strftime("%d/%m/%Y às %H:%M")
                nome_mes_p = MESES_PT_PDF.get(mes_num, str(mes_num))

                # ── Recalcula tabelas usando df_gav já disponível no escopo ──
                # tab_vend e tab_lid já foram calculados mais acima na função
                # Precisamos recalculá-los aqui pois estamos num bloco if
                from data_loader import apply_filters as _af2
                _mes_str = mes_sel
                _df_g = _af2(raw.copy(), _mes_str, ["NOVO","ADITIVO"], parceiro_sel)
                if not bko.empty and "pedido" in _df_g.columns:
                    _bk2 = bko.copy()
                    _df_g["pedido"] = _df_g["pedido"].apply(_norm_pedido)
                    _bk2["pedido"]  = _bk2["pedido"].apply(_norm_pedido)
                    _df_g = _df_g.merge(_bk2[["pedido","vendedor_real","lider"]], on="pedido", how="left")
                ...
                _atv = _df_g[_df_g["mes_ativacao"] == _mes_str].copy()
                ...
                _col = _lc()
                _metas = dict(zip(_col["vendedor"].str.upper(), _col["meta"])) if not _col.empty else {}
                tv["meta"] = tv["vendedor_real"].apply(lambda v: _metas.get(_s(v).upper(), 850.0))
                ...
                total_meta_p  = tv["meta"].sum()
                total_ativo_p = tv["real_ativo"].sum()
                total_tram_p  = tv["tramitando"].sum()
                total_proj_p  = total_ativo_p + total_tram_p
                pct_g_p = round(total_ativo_p/total_meta_p*100,1) if total_meta_p > 0 else 0
                n_vend_p = len(tv)
                ...
                s_tit = ParagraphStyle("t", fontSize=15, textColor=_colors.white,
                                       fontName="Helvetica-Bold", spaceAfter=3)
                s_sub = ParagraphStyle("s", fontSize=7,  textColor=COR["sub"],
                                       fontName="Helvetica", spaceAfter=10)
                s_sec = ParagraphStyle("sc", fontSize=9, textColor=COR["hdr_tx"],
                                       fontName="Helvetica-Bold", spaceBefore=12, spaceAfter=5)

                story = []
                story.append(Paragraph(f"GESTÃO À VISTA — {nome_mes_p.upper()} {ano_num}", s_tit))
                story.append(Paragraph(
                    f"Connect Group · TIM Empresas · Receita Contratada · {agora_pdf}", s_sub))
                story.append(HRFlowable(width="100%", thickness=1, color=COR["borda"], spaceAfter=8))

                # KPIs
                cor_g = _cpct(pct_g_p)
                kpi_d = [
                    ["🎯 Meta Total","✅ Real Ativado","🔄 Tramitando","📈 Projeção","⚡ % Atingimento"],
                    [_fmt_r(total_meta_p), _fmt_r(total_ativo_p), _fmt_r(total_tram_p), _fmt_r(total_proj_p), f"{pct_g_p:.1f}%"],
                    [f"{n_vend_p} vendedores","receita ativada","pipeline","ativo + pipeline","real / meta"],
                ]
                cw = doc.width / 5
                kt = Table(kpi_d, colWidths=[cw]*5, rowHeights=[16,22,12])
                kt.setStyle(TableStyle([
                    ("BACKGROUND",(0,0),(-1,0),COR["hdr_bg"]),("TEXTCOLOR",(0,0),(-1,0),COR["hdr_tx"]),
                    ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,0),7),
                    ("BACKGROUND",(0,1),(-1,1),COR["row_b"]),("TEXTCOLOR",(0,1),(-1,1),_colors.white),
                    ("FONTNAME",(0,1),(-1,1),"Helvetica-Bold"),("FONTSIZE",(0,1),(-1,1),10),
                    ("TEXTCOLOR",(4,1),(4,1),cor_g),
                    ("BACKGROUND",(0,2),(-1,2),COR["row_a"]),("TEXTCOLOR",(0,2),(-1,2),COR["sub"]),
                    ("FONTSIZE",(0,2),(-1,2),6),
                    ("ALIGN",(0,0),(-1,-1),"CENTER"),("VALIGN",(0,0),(-1,-1),"MIDDLE"),
                    ("BOX",(0,0),(-1,-1),0.5,COR["borda"]),("INNERGRID",(0,0),(-1,-1),0.3,COR["borda"]),
                    ("TOPPADDING",(0,0),(-1,-1),4),("BOTTOMPADDING",(0,0),(-1,-1),4),
                ]))
                story.append(kt)
                story.append(Spacer(1,10))

                def _build_table(rows_data, col_widths, style_extra=[]):
                    t = Table(rows_data, colWidths=col_widths)
                    base = [
                        ("BACKGROUND",(0,0),(-1,0),COR["hdr_bg"]),
                        ("TEXTCOLOR",(0,0),(-1,0),COR["hdr_tx"]),
                        ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),
                        ("FONTSIZE",(0,0),(-1,0),7),
                        ("ALIGN",(0,0),(-1,0),"CENTER"),
                        ("BOX",(0,0),(-1,-1),0.5,COR["borda"]),
                        ("INNERGRID",(0,0),(-1,-1),0.3,COR["borda"]),
                        ("TOPPADDING",(0,0),(-1,-1),4),("BOTTOMPADDING",(0,0),(-1,-1),4),
                ...
                # Tabela vendedores
                story.append(Paragraph("👤  Por Vendedor", s_sec))
                cw_v = [6.5*cm,3.5*cm,2.8*cm,2.8*cm,2.8*cm,2.8*cm,2.2*cm,1.5*cm]
                hdr_v = ["Vendedor","Líder / Equipe","Meta (R$)","✅ Real Ativado","🔄 Tramitando","📈 Projeção","% Ating.","Status"]
                rows_v = [hdr_v]
                sx_v = []
                for i, row in tv.iterrows():
                    ri = i+1
                    bg = COR["row_a"] if i%2==0 else COR["row_b"]
                    pct_v = row["pct"]
                    sx_v += [
                        ("BACKGROUND",(0,ri),(-1,ri),bg),("TEXTCOLOR",(0,ri),(-1,ri),COR["texto"]),
                        ("FONTNAME",(0,ri),(0,ri),"Helvetica-Bold"),
                        ("TEXTCOLOR",(3,ri),(3,ri),COR["verde"]),
                        ("TEXTCOLOR",(4,ri),(4,ri),COR["azul"]),
                        ("TEXTCOLOR",(5,ri),(5,ri),COR["roxo"]),
                        ("TEXTCOLOR",(6,ri),(6,ri),_cpct(pct_v)),
                        ("FONTNAME",(6,ri),(6,ri),"Helvetica-Bold"),
                        ("ALIGN",(2,ri),(-1,ri),"RIGHT"),
                    ]
                    rows_v.append([
                        row["vendedor_real"], row["lider"],
                        _fmt_r(row["meta"]) if row["meta"]>0 else "—",
                        _fmt_r(row["real_ativo"]), _fmt_r(row["tramitando"]),
                        _fmt_r(row["projecao"]),
                ...
                ri_t = len(rows_v)
                sx_v += [
                    ("BACKGROUND",(0,ri_t),(-1,ri_t),COR["tot_bg"]),
                    ("TEXTCOLOR",(0,ri_t),(-1,ri_t),COR["tot_tx"]),
                    ("FONTNAME",(0,ri_t),(-1,ri_t),"Helvetica-Bold"),
                    ("ALIGN",(2,ri_t),(-1,ri_t),"RIGHT"),
                    ("LINEABOVE",(0,ri_t),(-1,ri_t),1.5,COR["acc"]),
                ]
                rows_v.append([f"🏁 TOTAL ({n_vend_p} vend.)","",
                    _fmt_r(total_meta_p),_fmt_r(total_ativo_p),
                    _fmt_r(total_tram_p),_fmt_r(total_proj_p),
                    f"{pct_g_p:.1f}%",_semf(pct_g_p)])
                story.append(_build_table(rows_v, cw_v, sx_v))
                story.append(Spacer(1,14))

                # Divisória
                story.append(HRFlowable(width="100%",thickness=1,color=COR["acc"],spaceAfter=6))
                story.append(Paragraph("👥  Consolidado por Líder", s_sec))

                # Tabela líderes
                cw_l = [6*cm,2.5*cm,3.2*cm,3.2*cm,3.2*cm,3.2*cm,2.5*cm,1.5*cm]
                hdr_l = ["Líder / Equipe","Vendedores","Meta (R$)","✅ Real Ativado","🔄 Tramitando","📈 Projeção","% Ating.","Status"]
                rows_l = [hdr_l]
                sx_l = []
                for i, row in tl.iterrows():
                    ...
                    sx_l += [
                        ...
                        ("TEXTCOLOR",(4,ri),(4,ri),COR["azul"]),
                        ("TEXTCOLOR",(5,ri),(5,ri),COR["roxo"]),
                        ("TEXTCOLOR",(6,ri),(6,ri),_cpct(pct_l)),
                        ("FONTNAME",(6,ri),(6,ri),"Helvetica-Bold"),
                        ("ALIGN",(1,ri),(-1,ri),"RIGHT"),
                    ]
                    rows_l.append([
                        row["lider"], f"{int(row['n'])} vend.",
                        _fmt_r(row["meta"]) if row["meta"]>0 else "—",
                        _fmt_r(row["real_ativo"]), _fmt_r(row["tramitando"]),
                        _fmt_r(row["projecao"]),
                        f"{pct_l:.1f}%" if pct_l is not None else "—",
                        _semf(pct_l),
                    ])
                ri_tl = len(rows_l)
                sx_l += [
                    ("BACKGROUND",(0,ri_tl),(-1,ri_tl),COR["tot_bg"]),
                    ("TEXTCOLOR",(0,ri_tl),(-1,ri_tl),COR["tot_tx"]),
                    ("FONTNAME",(0,ri_tl),(-1,ri_tl),"Helvetica-Bold"),
                    ("ALIGN",(1,ri_tl),(-1,ri_tl),"RIGHT"),
                    ("LINEABOVE",(0,ri_tl),(-1,ri_tl),1.5,COR["acc"]),
                ]
                rows_l.append(["🏁 TOTAL GERAL",f"{n_vend_p}",
                    _fmt_r(total_meta_p),_fmt_r(total_ativo_p),
                    _fmt_r(total_tram_p),_fmt_r(total_proj_p),
                    f"{pct_g_p:.1f}%",_semf(pct_g_p)])
                story.append(_build_table(rows_l, cw_l, sx_l))

                # Rodapé
                story.append(Spacer(1,8))
                story.append(HRFlowable(width="100%",thickness=0.5,color=COR["borda"]))
                story.append(Paragraph(
                    f"Connect Group · TIM Empresas · {nome_mes_p} {ano_num} · Gerado em {agora_pdf}",
                    ParagraphStyle("rod",fontSize=6,textColor=COR["sub"],fontName="Helvetica",spaceBefore=3)
                ))
                ...
                if not token:
                    ...
                    semf_g = _semf(pct_g_p)
                    caption = (
                        f"{semf_g} *Gestão à Vista — {nome_mes_p} {ano_num}*\n"
                        f"📊 Ativado: *{_fmt_r(total_ativo_p)}* de {_fmt_r(total_meta_p)} ({pct_g_p:.1f}%)\n"
                    ...
                    for cid in chat_ids:
                        ...
                        r = _req2.post(
                            ...
                            files={"document": (f"gestao_vista_{nome_mes_p.lower()}_{ano_num}.pdf", buf2, "application/pdf")},
                            timeout=30
                        )
                ...
                st.error("❌ Biblioteca `reportlab` não instalada. Adicione ao requirements.txt: `reportlab`")
            except Exception as e:
                st.error(f"❌ Erro ao gerar/enviar PDF: {e}")
